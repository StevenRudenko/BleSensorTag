...
public class BluetoothLeService extends Service {
    ...
    private final BluetoothGattQuery gattQuery = new BluetoothGattQuery();
    ...
    private BluetoothGattQuery.SensorAction currentAction;
    ...
    private void execute() {
        ...
        if (!gattQuery.isEmpty()) {
            final BluetoothGattQuery.SensorAction action = gattQuery.pop();
            currentAction = action;
            action.sensor.execute(mBluetoothGatt, action.action);
        }
    }
    ...
    private void broadcastUpdate(final String action,
        ...
        if (sensor != null) {
            final String text = sensor.toString(characteristic);
     ...
     */
    public void setCharacteristicNotification(TiSensor<?> sensor, boolean enabled) {
        if (sensor == null)
            return;
        ...
        if (enabled) {
            ...
            gattQuery.push(sensor, TiSensor.ExecuteAction.BEGIN_NOTIFY);
        } else {
            gattQuery.push(sensor, TiSensor.ExecuteAction.END_NOTIFY);
            if (sensor.isTurnable()) {
                gattQuery.push(sensor, TiSensor.ExecuteAction.TUNN_OFF);
            }
        }
